  ...
  try {
    ...
    for (const sched of schedResult.rows) {
      ...
      if (sched.is_time_type) {
        const minutes = Math.round((timeVal || 0) / 60);
        sched.display_detail = `${minutes}분 × ${timeCount}세트`;
      } else {
...
app.patch("/api/schedule/:scheduleId/time-sets", async (req, res) => {
  ...
  try {
    ...
    for (const set of setList) {
      ...
      await client.query(`
        INSERT INTO exercise_time (schedule_id, set_number, elapsed_time_millis, weight, is_completed)
        VALUES ($1, $2, $3, $4, false)
      `, [scheduleId, set.set_number, millis, set.weight]);
    }
    ...
    console.error("❌ PATCH /time-sets 실패:", e);
    await client.query("ROLLBACK");
    res.status(500).json({ message: "세트 저장 실패" });
  } finally {
    client.release();
  }
});
